...
{
	AddBasicDrawableMessage::AddBasicDrawableMessage(std::shared_ptr<CHL::Model> model,
	...
	ChangeRastersizerState::ChangeRastersizerState(std::string ID, CULL_MODE cullMode, FILL_MODE fillMode)
	{
		this->ID = ID;
		this->cullMode = cullMode;
		this->fillMode = fillMode;
	...
	Message::Status ChangeRastersizerState::Work()
	{
		std::lock_guard<std::mutex> lock(GraphicManager::GetInstance().mutex);
	...
	ReleaseBasicDrawable::ReleaseBasicDrawable(std::string ID)
	{
		this->ID = ID;
	}

	Message::Status ReleaseBasicDrawable::Work()
	{
		std::lock_guard<std::mutex> lock(GraphicManager::GetInstance().mutex);
