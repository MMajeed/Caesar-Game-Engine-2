...
std::shared_ptr<Object> CameraINFO::Get(const std::string& ID)
{
	...
	if(!returnValue)
	{
			 if(ID == Keys::Camera::EYE){ returnValue = GenericObj<CML::Vec4>::CreateNew(this->Eye); }
		else if(ID == Keys::Camera::TARGETMAGNITUDE){ returnValue = GenericObj<CML::Vec4>::CreateNew(this->TargetMagnitude); }
		else if(ID == Keys::Camera::UP){ returnValue = GenericObj<CML::Vec4>::CreateNew(this->Up); }
		...
		else if(ID == Keys::Camera::CLEARCOLOR){ returnValue = GenericObj<CML::Vec4>::CreateNew(this->ClearColor); }
...
void CameraINFO::Set(const std::string& ID, std::shared_ptr<Object> obj)
{
	INFO::Set(ID, obj);
		 ...
		 if(ID == Keys::Camera::EYE){ this->Eye = GenericObj<CML::Vec4>::GetValue(obj); }
	else if(ID == Keys::Camera::TARGETMAGNITUDE){ this->TargetMagnitude = GenericObj<CML::Vec4>::GetValue(obj); }
	else if(ID == Keys::Camera::UP){ this->Up = GenericObj<CML::Vec4>::GetValue(obj); }
	...
	else if(ID == Keys::Camera::CLEARCOLOR){ this->ClearColor = GenericObj<CML::Vec4>::GetValue(obj); }
