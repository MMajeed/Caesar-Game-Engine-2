...
CHL::Matrix4x4 SpotLight::GetViewMatrix(TypedefObject::ObjectInfo& light)
{
	...
	vEye = GenericObj<CHL::Vec4>::GetValue(light[Keys::Light::POSITION]);
	vT(0) = 0.0; vT(1) = 0.0; vT(2) = 1.0; vT(3) = 0.0;
	vUp(0) = 0.0; vUp(1) = 1.0; vUp(2) = 0.0; vUp(3) = 0.0;

	pitch = GenericObj<CHL::Vec4>::GetValue(light[Keys::Light::DIRECTION])(0);
	yaw = GenericObj<CHL::Vec4>::GetValue(light[Keys::Light::DIRECTION])(1);
	roll = GenericObj<CHL::Vec4>::GetValue(light[Keys::Light::DIRECTION])(2);
...
CHL::Matrix4x4 SpotLight::GetPrespectiveMatrix(TypedefObject::ObjectInfo& light)
{
	...
	double width = 2048;
	double nearZ = 1.0;
	double farZ = GenericObj<double>::GetValue(light.find(Keys::Light::RANGE)->second);
